    ...
    if src_root and not resume_sid:
        ...
        if not recon_reply:
            ...
            recon_question = (
                ...
                "+ its EXACT pinned version. Then — from the sanitizer/"
                "parser SOURCE you can read (authoritative; web docs may "
                "supplement) — "
                "enumerate the bypass primitives THAT code fails to cover, "
                "ranked by applicability, each verified against the real "
                "predicate, and the viable EXFIL channel for the resulting "
                ...
                "technique is dead'. This applies to the EXFIL channel and "
                "the URL/scheme too: a banned WORD or char inside a hostname "
                "/ scheme / URI (a blocked scheme keyword, a dotted host, a "
                "non-80 port) is a TYPING limit, NOT proof the channel is "
                "dead — a host/scheme/keyword hidden in an encoded literal "
                "and decoded at runtime is never typed, so do NOT mark an "
                "exfil URI/scheme 'dead' on a word/char ban alone. If a "
                "scheme/sink might instead be blocked at EXECUTION (e.g. a "
                "browser navigation policy), record that as a SEPARATE, "
                "TESTABLE hypothesis — never conflate 'can't type it' with "
                "'won't run'; the latter must be execution-checked, not "
                "assumed. Sources + facts only; main does the "
