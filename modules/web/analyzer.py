        ...
        if not recon_reply:
            ...
            if _fw["staged"]:
                ...
                _fw_recon_section = (
                    ...
                    "For the pinned server + framework, state how a RESPONSE "
                    "is serialized and EXACTLY what is validated/rejected vs "
                    "passed through — compare header NAMES vs header VALUES, "
                    "the body, and redirect targets. Quote the validating "
                    "code file:line (the header-encode loop + the regex/check "
                    "it applies). If an attacker-controlled channel (header "
                    "dict, redirect URL, template kwargs) reaches a sink, say "
                    "whether THAT channel is actually guarded or only another "
                    "one is.\n"
                )
            recon_question = (
