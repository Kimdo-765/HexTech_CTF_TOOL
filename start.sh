...
if [ "$EK" != "0" ] && [ -e /dev/kvm ]; then
  if [ ! -f "$OVR" ] || _is_gen_ovr; then
    cat > "$OVR" <<YAML
$KVM_MARKER
# Regenerated on every ./start.sh. Set ENABLE_KVM=0 in .env (or export it in
# your shell) to disable, or replace the marker line above to hand-manage this
# file. Passing /dev/kvm lets the worker's qemu-system-x86_64 use -enable-kvm
# for kernel pwn. The ':rw' drops the cgroup mknod ('m') bit qemu never needs.
services:
  worker:
    devices:
      - /dev/kvm:/dev/kvm:rw
