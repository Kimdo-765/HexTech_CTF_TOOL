...
# The quota UI must not ship behind a stale browser cache. This branch pinned a
# hand-typed buster (`?v=20260809-provider-quota`), which is the exact failure
# main replaced: a literal that nobody remembers to bump, so a shipped UI change
# looks like it never shipped. main derives the buster from the bytes of app.js
# + style.css, so it moves whenever this feature's JS moves — no discipline
# required. `scripts/test_role_provider_ui.js` owns the recomputation and prints
# the value to use; here we only pin the SHAPE, so a hand-typed string cannot
# come back.
import re as _re

_buster = _re.search(r'app\.js\?v=([^"\']+)', index_html)
check(
    "app.js is cache-busted",
    _buster is not None,
)
check(
    "...by a content-derived buster, not a hand-typed literal",
    bool(_buster) and bool(_re.fullmatch(r"a[0-9a-f]{8}", _buster.group(1))),
)
