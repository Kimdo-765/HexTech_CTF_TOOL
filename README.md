...
1. **`.env`** — read at container startup, applied to compose substitution:
   ...
   | `AGENT_OUTPUT_LANGUAGE` | `auto` | human-facing prose from Main, Recon/subagents, Judge, Reviewer, and Report: `auto`, `ko`, or `en`. Settings supplies the global default and every job form can override it. The effective value is snapshotted at job creation; code, commands, paths, errors, evidence, schema keys, and flags remain verbatim. |
